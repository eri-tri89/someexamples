...
import application.model.RowChanger;
import application.model.RowFile;
import application.model.RowState;
...
public class MainController {
	...
	public void initialize() {	
		...
		tableView.setRowFactory(new RowChanger());
		tableView.getSelectionModel().selectedItemProperty().addListener((obs,old,nev)->{
			...
			if(rf != null){
				if(rf.getState() == RowState.SELECTED){
					System.out.println("Before "+rf.getState());
					rf.setState(RowState.NO_SELECTED);
					System.out.println("After "+nev.getState());
				}else{
					System.out.println("Before "+rf.getState());
					rf.setState(RowState.SELECTED);
					System.out.println("After "+nev.getState());
				}
